...
const Carousel = () => {
  ...
  return (
    <section id="home">
      <div className="w-full max-w-full mx-auto pt-5 overflow-hidden bg-dark">
        <Slider {...settings}>
          {
              trendingMovies.map((movie) => (
                  <div className="relative" key={movie.id}>
                    <div className='w-full max-w-screen max-h-screen bg-cover bg-center top-0 left-0 z-0 mt-6'>
                      <img 
                      src={`${process.env.REACT_APP_BASEIMGURL}original${movie.backdrop_path}`}
                      alt="Slide 1" 
                      className="w-full h-auto object-cover" />
                      <div class="absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-transparent to-gray-700 mt-5"></div>
                    </div>

                    {/* Black Overlay */}
                    <div className="absolute top-0 left-0 w-full h-screen bg-black opacity-50 z-5"></div>

                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center z-10 text-white w-4/5 md:w-1/2">
